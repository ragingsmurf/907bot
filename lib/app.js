...
router.post('/sms', function*(next) {
  ...
  let handled = yield cookie.Parser(req, res, frm, txt, ckz);
  ...
  if (!handled) {
    ...
    if (!registered) {
      l.c(`Unknown user from number ${frm}`);
      // 2.1 Setup registration process.
      user.register(req, res, frm, ckz, txt);
    } else {
      l.c(`Request is from a returning user.`);
      // Get cookie state.
      let state = require('./../modules/cookie.state')(ckz);
