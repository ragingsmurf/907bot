...
router.post('/sms', function*(next) {
  ...
  // l.c('text:' + JSON.stringify(this.req));

  // let time = yield data.knwl('7pm');
  l.c(`Received /sms POST from (${frm}).`);

  // 1. Parse cookie state (pre-existing )
  // yield cookie.Parser(req, res, frm, txt, ckz);

  // 2. Check the incoming phone number, are they an existing user?
  let registered = yield user.registered(req, res, frm, ckz);
  if (!registered) {
    l.c(`Unregistered number (${frm}); user.register.`);
    // 2.1 Register the current phone number.
    yield user.register(req, res, frm, ckz, txt);
  } else {
    l.c(`Request is from a returning user.`);
    let state = state.get(ckz);
    if (state != undefined) {
